...
def check_ssh_port(connections):
    ssh_on_22 = any(c["local_port"] == 22 and c["state"] == "LISTEN" for c in connections)
    return {
        "name": "SSH Port Check",
        "passed": not ssh_on_22,
        "details": (
            "SSH on port 22 (consider changing)"
            if ssh_on_22
            else "SSH not on default port or not running"
        ),
    }
...
def check_sensitive_external(connections):
    ...
    for conn in connections:
        if (
            ...
            and conn["local_port"] in sensitive_ports
            and conn["remote_ip"] not in ["127.0.0.1", "0.0.0.0", "::1", "::"]
        ):
    ...
    return {
        ...
        "details": (
            f"External access: {', '.join(external_sensitive[:5])}"
            if external_sensitive
            else "No external sensitive port access"
        ),
    }
...
def check_outbound_suspicious(connections):
    ...
    suspicious = []
    for conn in connections:
        if conn["state"] == "ESTABLISHED" and conn["remote_port"] in suspicious_ports:
    ...
    return {
        ...
        "details": (
            ...
            if suspicious
            else "No suspicious outbound connections"
        ),
    }
