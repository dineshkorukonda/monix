...
The suite is **integration-leaning**: Django tests hit the ORM and views; scoring and checker tests use structured fixtures and assert **numeric or structural outcomes**, not only status strings. Prefer adding tests when fixing bugs or changing contracts; avoid duplicate cases that only reassert the same branch with different literals unless they document a real edge case.
