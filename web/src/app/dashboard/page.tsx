...
import { Server, Activity, ArrowRight, ShieldAlert, Globe, Plus } from "lucide-react";
    ...
    fetch("/api/targets/")
      ...
      .catch(() => {
        setProjects([{
          ...
          activity: "Django backend is unreachable. Start the backend to view live targets.",
          status: "Warning",
          lastScan: "Never",
          score: 0
        }]);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">Monitored Targets</h2>
          <p className="text-sm text-muted-foreground mt-1">Select a target to view its complete security report engine.</p>
        </div>
        <Button asChild className="font-semibold gap-2 shadow-sm bg-foreground text-background hover:bg-foreground/90">
            ...
            <div className="space-y-5">
              ...
              <div className="flex justify-between items-start">
                <div className="flex gap-3.5 items-center">
                  ...
                  <div className="truncate pr-4">
                    <h3 className="text-[15px] font-semibold tracking-tight truncate group-hover:underline underline-offset-2">{project.name}</h3>
                    <p className="text-xs text-muted-foreground mt-0.5 truncate">{project.url}</p>
                  </div>
                </div>
              ...
              <div className="pt-1">
                <p className="text-[13px] font-medium leading-snug flex gap-1.5 items-start">
                  <ShieldAlert className={`h-4 w-4 shrink-0 mt-0.5 ${project.status === 'Warning' ? 'text-amber-500' : 'text-emerald-500'}`} />
                  <span className="truncate whitespace-normal line-clamp-2">{project.activity}</span>
                </p>
