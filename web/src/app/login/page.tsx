    ...
    <div className="min-h-screen w-full lg:grid lg:grid-cols-2 bg-background font-sans">
      ...
      <div className="hidden lg:flex flex-col justify-center items-center overflow-hidden bg-muted relative">
        <div className="absolute inset-0 bg-sidebar">
          ...
          <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-accent/20 rounded-full blur-[80px] animate-pulse" style={{ animationDelay: "2s" }}></div>
        </div>
        
        <Link href="/" className="absolute top-10 left-10 z-10 flex items-center font-bold text-3xl tracking-tighter text-foreground hover:opacity-80 transition-opacity">
          Monix.
        </Link>
      </div>
      ...
      <div className="flex flex-col items-center justify-center px-6 md:px-10 lg:px-16 h-screen overflow-y-auto">
        <div className="w-full max-w-sm mx-auto space-y-8">
          <div className="flex flex-col space-y-2 text-center lg:text-left">
            <Link href="/" className="lg:hidden mb-6 font-bold text-2xl tracking-tighter text-foreground text-center">
              Monix.
            </Link>
            ...
            <p className="text-sm text-muted-foreground">
              ...
              {mode === "signup" && "Enter your details to register your new monix profile."}
              {mode === "reset" && "We will email you a secure link to reset your key."}
            </p>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <Button type="submit" className="w-full font-semibold transition-all h-11 text-[15px] shadow-sm active:scale-[0.98]">
                {mode === "login" && "Sign In"}
              ...
              <div className="text-center text-sm text-muted-foreground mt-6 space-y-4">
                {mode === "login" && (
                  <div>
                    ...
                    <button type="button" onClick={() => setMode("signup")} className="text-foreground hover:underline font-medium">Sign up</button>
                  </div>
                )}
                {mode === "signup" && (
                  <div>
                    Already have an account?{" "}
                    <button type="button" onClick={() => setMode("login")} className="text-foreground hover:underline font-medium">Sign in</button>
                  </div>
                )}
                {mode === "reset" && (
                  <div>
